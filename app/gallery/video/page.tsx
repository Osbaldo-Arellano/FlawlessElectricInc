...
function VideoPageInner() {
  ...
  const [index, setIndex] = useState(
    Math.max(0, Math.min(startIndex, videos.length - 1)),
  );
      ...
      <header className="sticky top-0 z-50 bg-black/80 backdrop-blur-md border-b border-white/5">
        <div className="flex items-center justify-between px-4 py-3 max-w-screen-2xl mx-auto">
          ...
          <div className="text-center min-w-0 px-4">
            ...
            {video.category && (
              <p className="text-xs text-white/40">{video.category}</p>
            )}
          </div>
            ...
            >
              {videos.map((thumbVideo, i) => (
                <button
                  ...
                  onClick={() => goTo(i)}
                  className={cn(
                    "relative shrink-0 w-20 h-14 sm:w-24 sm:h-16 rounded-md overflow-hidden transition-all duration-200 bg-white/5",
                    i === index
